...
@RestController
public class DownloadMeitu {

    /**
     * 抓取所有的妹子图
     * */
    public void downloadAllMeiZiTuPics(int page) {

        String url = "https://api.meizitu.net/wp-json/wp/v2/comments?post=174285&per_page=20&page=" + page;
        ...
        // json解析
        List<JiePai> meiziItems = JSONObject.parseArray(result, JiePai.class);

        if (meiziItems != null && meiziItems.size() > 0) {
            for (int i = 0; i < meiziItems.size(); i++) {
                JiePai item = meiziItems.get(i);
                downloadImg(item.getImg_src(), item.getId());
            }

            downloadAllMeiZiTuPics(page + 1);
        } else {
            System.out.println("==============================");
            System.out.println("==============================");
            System.out.println("==============================");
